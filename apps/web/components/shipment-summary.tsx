...
export function ShipmentSummary({ order }: { order: OrderView }) {
  ...
  return (
    <div className="mt-4 rounded-[16px] border border-border bg-surface p-3 text-sm">
      <p>
        Entrega: {SHIPPING_METHOD_LABELS[order.shippingMethod]}
        {shipment ? ` · ${SHIPMENT_STATUS_LABELS[shipment.status]}` : ""}
      </p>
      {order.shippingClp > 0 ? <p className="text-text-muted">Costo de envío incluido en el total.</p> : null}
