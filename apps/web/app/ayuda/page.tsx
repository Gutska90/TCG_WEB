        ...
        {HELP_FAQS.map((section) => (
          <article key={section.heading} className="mt-6">
            ...
            {section.paragraphs.map((paragraph) => (
              <p key={paragraph.slice(0, 40)} className="mt-2 leading-relaxed text-neutral-800">
                {paragraph}
              </p>
            ))}
          </article>
      ...
      <section className="mt-12">
        ...
        <form onSubmit={(event) => void onSubmit(event)} className="mt-4 grid gap-3">
          <label className="flex flex-col gap-1 text-sm">
            Categoría
            <select name="category" required className="rounded border border-neutral-300 px-3 py-2">
          ...
          {ok ? <p className="text-sm text-emerald-800">Gracias. Recibimos tu mensaje.</p> : null}
          <button
            type="submit"
            disabled={pending}
